...
var transcode_sro_word_to_syllabics = function (sro_word, hyphen, sandhi) {
  ...
  var lookup = Object.new(sro2syllabics_lookup)
  ...
  while (match) {
    ...
    if (sandhi && onset !== null) {
      ...
      next_syllable_pos = syllable.length
    } else {
    ...
    to_transcribe = to_transcribe.__getslice__(next_syllable_pos, null, 1)
    match = sro_pattern.match(to_transcribe)
  }
  if (parts.__getslice__(-(2), null, 1) == ['ᐦ', 'ᐠ']) {
    parts.__setslice__(-(2), null, null, [sro2syllabics_lookup['hk']])
  }
  return ''.join(parts)
}
